...
A third pass (2026-09-22, the "find maximum venues" expansion) ran 5 parallel research agents, one per geographic zone (north/Yonge-Church-Wellesley, east/Cabbagetown-Parliament, south/Moss Park, west/College Park, and a central re-sweep) covering the same 1km radius, adding 63 new venues and 89 new dishes on top of the original 25. Two honesty caveats carry over from that pass: some zones lost Google Maps browser access mid-run to tool contention and fell back to web search, so their venue coordinates are street-geocoded estimates rather than GPS pins (not survey-grade, but not fabricated); and the central zone's dish prices are mostly the midpoint of a Google Maps listing's price range rather than a specific named dish's real price; both cases are marked `confidence: "low"` and say so directly in the dish's `note` field. Duplicate venues found in more than one zone (or already present from earlier passes) were merged, keeping whichever source had the better-verified price and coordinates.
